...
describe('registry (manifests → menú/rutas)', () => {
  it('admin accede a todos los módulos registrados, incl. orientador-ia (admin también entra ahí desde 2026-07-15), gestor-proyectos, focus-flow y factory-console (D-030)', () => {
  ...
  it('un usuario sin admin no accede a core-admin (moodle-insights, gestor-proyectos y focus-flow sí, sin requiredRoles)', () => {
    const ids = accessibleModules(plain).map((mod) => mod.manifest.id);
    expect(ids).toEqual(['hello', 'moodle-insights', 'gestor-proyectos', 'focus-flow']);
  });
  ...
  it('un admin de Aspasia (orientador_admin) ve orientador-ia pero no core-admin ni la fábrica', () => {
    ...
    expect(ids).toEqual(['hello', 'moodle-insights', 'orientador-ia', 'gestor-proyectos', 'focus-flow']);
  });
